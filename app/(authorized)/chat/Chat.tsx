...
import { useChatSocket, IChat, sendMessage } from "./Socket";
...
function ChatDropDown({ open }: { open: boolean }) {
  return (
    ...
    >
      <div className="flex justify-center items-center h-8 border-b">신고하기</div>
...
function ChatFileForm({ open }: { open: boolean }) {
  return (
    <div className="bg-[#E6E6E6] flex justify-center items-center h-16" style={{ display: open ? "block" : "none" }}>
...
function ChatRoom({
  ...
  return (
    <div className="flex flex-col h-fit w-[19rem] rounded-md border bg-[#E3F8FF] mt-auto">
      <div>
        <div className="chat-header bg-[#FFDD83] flex items-center text-white" onClick={() => handle(_index)}>
          <div className="opposite-name grow text-xl flex justify-center items-center pl-4">{item.name}</div>
          <MoreHorizontal size={28} className="mr-4" onClick={() => setDropDownOpen((prev) => !prev)} />
        </div>
        <div className="right-1 top-7 absolute">
          <ChatDropDown open={dropDownOpen} />
        </div>
      </div>
      <div className="container flex flex-col grow h-72 justify-between" style={{ display: open ? "flex" : "none" }}>
        ...
        <div className="chat-foot flex flex-col">
          <div className="chat-textInput h-8 bg-white flex items-center justify-center">
            <Plus size={14} className="ml-2" onClick={() => setFileFormOpen((prev) => !prev)} />
            <input
              type="text"
  ...
  return (
    <div className="flex gap-x-2">
      {chatList?.map((item, index) => (
        <ChatRoom item={item} handle={handleChatOpen} key={index} _index={index} open={chatOpen[index]} />
      ))}
    </div>
  );
}
